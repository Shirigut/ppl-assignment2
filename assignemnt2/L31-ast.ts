// ===========================================================
// AST type models
import { map, zipWith, all} from "ramda";
...
const makeOkVarDecl = (param: string): Result<VarDecl> =>
    makeOk(makeVarDecl(param));


const makeMet = (method: Sexp): Result<Binding[]> => {

    if (!isGoodBindings(method)) {
        return makeFailure('Malformed bindings in "Class" expression');
    }
    const vars = map(b => b[0], method);

    const valsResult = mapResult(met => parseL31CExp(second(met)), method);
...
export const parseClassExp = (fields: Sexp, methods:Sexp):  Result<ClassExp> => {
    ...
    const varDeclsResult = mapResult(makeOkVarDecl, fields);
    return safe2((fie: VarDecl[], met: Binding[]) => makeOk(makeClassExp(fie, met)))
        (varDeclsResult, bind(makeMet(methods), (met:Binding[])=> 
        all(b => isProcExp(b.val), met)? makeOk(met): makeFailure('Malformed bindings in "Class" expression')));
}
...
const unparseClassExp = (cl: ClassExp) : string => 
    `(class (${map((f: VarDecl) => f.var, cl.fields).join(" ")}) (${map((m: Binding) => `(${m.var.var} ${unparseL31(m.val)})`, cl.methods).join(" ")}))`
